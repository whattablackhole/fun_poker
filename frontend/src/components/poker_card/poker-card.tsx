  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      if (cardValue !== undefined) {
        setShow("poker-card show");
      }
    }, 500);

    return () => clearTimeout(timer);
  }, [buttonId, cardSuit, cardValue]);
    ...
    <>
      <div className={show}>
        <div className="card-face card-front">
          {cardSuit !== undefined && (
            <div
              className="poker-card-value"
              style={{ color: getCardColor(cardSuit) }}
            >
          ...
          <div className="poker-card-image">
            <img
              src={getCardImage(cardSuit, cardValue)}
        ...
        <div className="card-face card-back">
          <img
            src={getCardImage(undefined, undefined)}
            className="poker-card-image"
          ></img>
        </div>
      </div>
