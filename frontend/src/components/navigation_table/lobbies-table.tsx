...
import { GameName, GameType, LobbyList } from "../../types/lobby";

function LobbiesTable({
  joinLobbyHandler,
}: {
  joinLobbyHandler: (...args: any) => void;
}) {
  ...
  const rows = rowsData.map((row, i) => {
    const tds = Object.values(row).map((value, y) => (
      <TableCell key={y}>{value}</TableCell>
    ));

    return (
      <TableRow key={i} style={{ border: "1px solid black" }}>
        {tds}
        <TableCell>
          <Button onClick={() => joinLobbyHandler(row.id)}>Join Lobby</Button>
        </TableCell>
      </TableRow>
    );
  });
